...
def plot_gammas(gammas=None, box_sizes=None, title=''):
    ...
    for i, gamma in enumerate(gammas):
        ...
        plt.plot(iv,gamma.mat[niw+1,:,niv].real,'o', ms = 1, label='Core:{} ;Urange{} '.format(box_sizes[i]['niv_core'],box_sizes[i]['niv_urange']))
    plt.legend()
    plt.xlabel(r'$\nu_n$')
    plt.ylabel(r'$\Gamma$')
...
# Plot Gamma:
fig = plot_gammas(gammas=gammas_magn,box_sizes=box_sizes, title='$\Gamma_{DMFT;magn}$')
