...
def load_ldga_n_an_v3(path=None):
    ...
    # = sigma_dga['dens'] + 3*sigma_dga['magn']  - 2*dmft_sde['magn'] + 0*dmft_sde['dens'] - dmft_sde['siw'] + siw_dmft_clip
    sigma = np.load(path + 'sigma.npy', allow_pickle=True)
    sigma_node = sigma[nk[0] // 4, nk[1] // 4, 0, niv_urange:]
    sigma_anti_node = sigma[nk[0] // 2, 0, 0, niv_urange:]
    w = (config.box.vn_urange[niv_urange:] * 2 + 1) * np.pi / config.sys.beta
...
fig = plt.figure()
plt.plot(dmc_an_065[:,0],rescal(dmc_an_065[:,1]), '-o', color = colors[4])
...
fig = plt.figure()
plt.plot(dmc_n_065[:,0],rescal(dmc_n_065[:,1]), '-o', color = colors[4])
