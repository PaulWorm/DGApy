...
sys.path.append('../src/')
sys.path.append(os.path.join(os.path.dirname(os.getcwd()), 'src'))  # dynamic path finding of src dir
import numpy as np
import Hr as hr_mod
import Hk as hamk
...
if socket.gethostname() == 'Julian-PC':
    names.input_path = 'D:/Research/HoleDopedCuprates/2DSquare_U8_tp-0.2_tpp0.1_beta10_n0.85/KonvergenceAnalysis/'
...
while n_iter < 2:
    ...
    chi_dga, vrg_dga = fp.dga_susceptibility(dga_conf=dga_conf, dmft_input=dmft1p, qiw_grid=qiw_grid.my_mesh,
                                             ...
                                             q_grid=dga_conf.q_grid, hr=dga_conf.sys.hr, sigma=sigma)
    ...
    sigma_rpa = sde.rpa_sde_wrapper(dga_conf=dga_conf, dmft_input=dmft1p, chi=chi_rpa, qiw_grid=qiw_grid_rpa,
                                    distributor=qiw_distributor_rpa, sigma=dmft1p['sloc'])
    if (comm.rank == 0): np.save(dga_conf.nam.output_path + 'sigma_rpa.npy', sigma_rpa, allow_pickle=True)

    sigma, sigma_nc = sde.build_dga_sigma(dga_conf=dga_conf, sigma_dga=sigma_dga, sigma_rpa=sigma_rpa,
                                          dmft_sde=dmft_sde,
                                          dmft1p=dmft1p)
