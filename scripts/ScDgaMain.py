...
sigma_input = dmft1p['sloc']
# Try self-consistency:
mixing_ratio = 0.5
for i in range(20):
    logger.log_cpu_time(task=f' Started Iteration: {i}')
    ldga = lambda_dga.LambdaDga(config=dga_conf, comm=comm, sigma_dmft=dmft1p['sloc'], sigma_start=sigma_input, gamma_magn=gamma_dmft['magn'],
                                gamma_dens=gamma_dmft['dens'])
    ...
    ldga.dga_bse_susceptibility(save_vrg=False)
    ...
    #sigma_input = np.mean(ldga.sigma,axis=(0,1,2))
    sigma_input = ldga.sigma * (mixing_ratio) + mf.cut_v(arr=sigma_input, niv_cut=dga_conf.box.niv_padded, axes=(-1,)) * (1-mixing_ratio)
