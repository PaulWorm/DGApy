...
def cont_bosonic_ind_list(mat=None, ind_list=None, v_real=None, beta=None, n_fit=20, alpha_det_method='historic', err=0.002,
                          use_preblur=False, bw=None, optimizer='newton'):
    ''' ind are the indizes on which the pade approximation shall be performed'''
    n_ind = len(ind_list)
    nw = np.size(v_real)
    ...
    if (bw == 0):
        use_preblur = False
    for i, ind in enumerate(ind_list):
        mat_cont[i, :] = cont_bosonic_single_ind(mat=mat[ind], v_real=v_real, beta=beta, n_fit=n_fit, alpha_det_method=alpha_det_method, err=err,
                                 use_preblur=use_preblur, bw=bw, optimizer=optimizer)

    return mat_cont


def cont_bosonic_single_ind(mat=None, v_real=None, beta=None, n_fit=20, alpha_det_method='historic', err=0.002, use_preblur=False,
                            bw=None, optimizer='newton', return_chi2=False):
    niv_mat = np.size(mat) // 2
    mat_plus = mat[niv_mat:niv_mat + n_fit+1]
    wn_plus = 1j * mf.wn(beta, n_fit, pos=True)

    problem = cont.AnalyticContinuationProblem(im_axis=wn_plus.imag, re_axis=v_real, im_data=mat_plus,
                                               kernel_mode="freq_bosonic",
                                               beta=beta)
    model = np.ones_like(v_real)
    # model = np.exp(-v_real ** 2 / 2 ** 2)
    model *= mat_plus[0].real/np.trapz(model, v_real)

    error_s = np.ones_like(wn_plus) * err
    sol, _ = problem.solve(method='maxent_svd', model=model, stdev=error_s, alpha_determination=alpha_det_method,
                         ...
                         alpha_start=1e12, alpha_end=1e-3, fit_position=2.)
    chi2 = sol.chi2
    cont_mat = sol.A_opt
    del problem, sol, _
    gc.collect()
...
def max_ent_chi_irrk(mat, k_grid, me_conf: config.MaxEntConfig, comm, bw):
    mpi_distributor = mpi_aux.MpiDistributor(ntasks=k_grid.nk_irr, comm=comm)
    my_ind = k_grid.irrk_ind_lin[mpi_distributor.my_slice]
    mat_cont = cont_bosonic_ind_list(mat=mat, ind_list=my_ind, v_real=me_conf.mesh,
                                     beta=me_conf.beta,
                                     n_fit=me_conf.n_fit_chi, err=me_conf.err_chi, alpha_det_method=me_conf.alpha_det_method,
                                     use_preblur=me_conf.use_preblur_chi, bw=bw, optimizer=me_conf.optimizer)
    mat_cont = mpi_distributor.allgather(rank_result=mat_cont)
    return mat_cont


def max_ent_irrk_bw_range_chi(chi, k_grid: bz.KGrid, me_conf: config.MaxEntConfig, comm, bw, logger=None,
                              name='',out_dir='./'):
    chi_irrk = k_grid.map_fbz2irrk(chi)
    chi_cont = max_ent_chi_irrk(chi_irrk, k_grid, me_conf, comm, bw)
    if (logger is not None): logger.log_cpu_time(task=' for Chi-{} MaxEnt done left are plots and gather '.format(name))

    if (logger is not None):  logger.log_cpu_time(task=' for Chi-{} Gather done left are plots '.format(name))
    if (comm.rank == 0):
        chi_cont = k_grid.map_irrk2fbz(chi_cont)

        plotting.plot_cont_fs_no_shift(output_path=out_dir,
                              name='chi_fermi_surface_' + name + '_cont_w0-bw{}'.format(bw),
                              gk=chi_cont, v_real=me_conf.mesh, k_grid=k_grid, w_plot=0)
        np.save(out_dir + 'chi_' + name + '_cont_fbz_bw{}.npy'.format(bw), chi_cont,
                allow_pickle=True)
...
def do_max_ent_on_ind_T(mat=None, ind_list=None, v_real=None, beta=None, n_fit=60, alpha_det_method='historic', err=1e-4,
                        use_preblur=False, bw=None, optimizer='newton'):
    ''' ind are the indizes on which the pade approximation shall be performed'''
    n_ind = len(ind_list)
    nw = np.size(v_real)
...
def max_ent_loc_bw_range(mat, me_conf: config.MaxEntConfig, name=''):
    ...
    np.save(me_conf.output_path_loc + 'w.npy', me_conf.mesh, allow_pickle=True)
    return bw_opt
...
def max_ent_irrk_bw_range_sigma(sigma: twop.SelfEnergy, k_grid: bz.KGrid, me_conf: config.MaxEntConfig, comm, bw, logger=None,
    ...
    if (comm.rank == 0):
        ...
        np.save(me_conf.output_path_nl_s + 'swk_' + name + '_cont_fbz_bw{}.npy'.format(bw), sigma_cont,
                allow_pickle=True)
        np.save(me_conf.output_path_nl_s + 'w.npy', me_conf.mesh, allow_pickle=True)
    return None
...
def max_ent_irrk_bw_range_green(green: twop.GreensFunction, k_grid: bz.KGrid, me_conf: config.MaxEntConfig, comm, bw, logger=None,
    ...
    if (comm.rank == 0):
        ...
        np.save(me_conf.output_path_nl_g + 'gwk_' + name + '_cont_fbz_bw{}.npy'.format(bw), green_cont,
                allow_pickle=True)
        np.save(me_conf.output_path_nl_g + 'w.npy', me_conf.mesh, allow_pickle=True)
