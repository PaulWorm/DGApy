# ------------------------------------------------ COMMENTS ------------------------------------------------------------
'''
    Module for the lambda correction routines.
'''
...
KNOWN_LAMBDA_CORRECTION_TYPES = ['spch', 'sp', 'none']
...
def lambda_correction(chi_lad_dens, chi_lad_magn, beta, chi_loc_dens, chi_loc_magn, lambda_corr='spch', verbose=False):
    if lambda_corr == 'spch':
    ...
    elif lambda_corr == 'none':
        return chi_lad_dens, chi_lad_magn, 0, 0
    else:
        raise NotImplementedError(f'Unknown lambda correction: {lambda_corr} ! Implemented is {KNOWN_LAMBDA_CORRECTION_TYPES}.')
