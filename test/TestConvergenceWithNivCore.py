...
def get_convergence_with_niv_core(input_path, input_type, fname_1p, fname_2p, niv_shell, niv_core_range, niw_core):
    ...
    for niv_core in niv_core_range:
        ...
        gamma_dens, gamma_magn, chi_dens, chi_magn, _, _, siw_sde_full = hlr.local_sde_from_g2(g2_dens, g2_magn, giwk_dmft,
                                                                                               dga_config,
                                                                                               dmft_input,
                                                                                               comm,
                                                                                               write_output=False)
    ...
    return siw_sde_range, gamma_dens_range, gamma_magn_range, chi_dens_range, chi_magn_range
...
niv_core_range = np.array([10,20,40,60])

niv_shell = 100
niw_core_1 = 30
siw_sde_1, gamma_dens_1, gamma_magn_1, chi_dens_1, chi_magn_1 = get_convergence_with_niv_core(input_path, input_type, fname_1p,
                                                                                               fname_2p, niv_shell,
                                                                                               niv_core_range,
                                                                                               niw_core_1)
...
line_colors = plt.cm.get_cmap('jet')(np.linspace(0, 1, len(niv_core_range)))
...
plt.tight_layout()
plt.savefig(fname=pdir + 'Siw_convergence_with_niv_core.png')
plt.show()