...
f1p = w2dyn_aux.W2dynFile(fname=path + fname1p)
...
f2p = w2dyn_aux_dga.W2dynG4iwFile(fname=path + fname2p)
niw_core = f2p.get_niw(channel='dens')
niw_core = 50
niv_invbse = niw_core
wn_core = mf.wn(n=niw_core)
g2_dens = fp.LocalFourPoint(channel='dens', mat=f2p.read_g2_iw(channel='dens', iw=wn_core), beta=dmft1p['beta'], wn=wn_core)
g2_magn = fp.LocalFourPoint(channel='magn', mat=f2p.read_g2_iw(channel='magn', iw=wn_core), beta=dmft1p['beta'], wn=wn_core)
f2p.close()
...
# Extract gamma:
chi0_urange = fp.BubbleGenerator(wn=wn_core, giwk_obj=giw_dmft)
chi0_core = fp.BubbleGenerator(wn=wn_core, giwk_obj=giw_dmft)
...
F_dens_pp = pv.ph2pp_iwn(mat_ph=F_dens, iwn_pp=0)
F_magn_pp = pv.ph2pp_iwn(mat_ph=F_magn, iwn_pp=0)
