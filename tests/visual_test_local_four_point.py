...
def chi_convergence_from_gammar_urange_niv_core(ddict, ek, niv_core, niv_shell, name):
    ...
    for niv in niv_core:
        ...
        gamma_dens = lfp.gamob2_from_gchir_urange(gchi_dens, gchi0_core, u)
        gamma_magn = lfp.gamob2_from_gchir_urange(gchi_magn, gchi0_core, u)
...
def chi_convergence_from_gammar_urange_niv_shell(ddict, ek, niv_core, niv_shell, name):
    ...
    for niv in niv_shell:
        ...
        gamma_dens = lfp.gamob2_from_gchir_urange(gchi_dens, gchi0_shell, u)
        gamma_magn = lfp.gamob2_from_gchir_urange(gchi_magn, gchi0_shell, u)
...
def test_vertex_frequency_convention(ddict, ek, count=1):
    ...
    wn = mf.wn(niw)
    g2_dens = lfp.LocalFourPoint(channel='dens', mat=g2_file.read_g2_iw(channel='dens', iw=wn), beta=beta, wn=wn)
    g2_magn = lfp.LocalFourPoint(channel='magn', mat=g2_file.read_g2_iw(channel='magn', iw=wn), beta=beta, wn=wn)
    gchi_dens = lfp.gchir_from_g2(g2_dens, g_loc)
    gchi_magn = lfp.gchir_from_g2(g2_magn, g_loc)
    chi0_gen = bub.BubbleGenerator(wn, giwk)
    gchi0_core = chi0_gen.get_gchi0(niv=gchi_dens.niv)
    F_dens = lfp.fob2_from_gchir(gchi_dens, gchi0_core)
    F_magn = lfp.fob2_from_gchir(gchi_magn, gchi0_core)

    gamma_dens = lfp.gamob2_from_gchir(gchi_dens, gchi0_core)
    gamma_magn = lfp.gamob2_from_gchir(gchi_magn, gchi0_core)
